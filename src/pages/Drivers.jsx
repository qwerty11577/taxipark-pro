...
export default function Drivers() {
    ...
    const filtered = drivers.filter(d => {
        const matchSearch = d.name.toLowerCase().includes(search.toLowerCase()) ||
            (d.car && d.car.toLowerCase().includes(search.toLowerCase()));
            ...
            {showModal && (
                <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(4px)" }}>
                    <div style={{ background: "#0f0f1a", border: "1px solid rgba(245,197,24,0.2)", borderRadius: "24px", padding: "40px", maxWidth: "480px", width: "90%" }}>
                        ...
                        {[
                            { label: "Имя и фамилия", key: "name", placeholder: "Алишер Каримов" },
                            { label: "Автомобиль", key: "car", placeholder: "Toyota Camry" },
                        ...
                        <div style={{ display: "flex", gap: "12px" }}>
                            ...
                            <button onClick={addDriver} style={{ flex: 1, padding: "14px", background: "linear-gradient(135deg, #f5c518, #ff8c00)", color: "#000", border: "none", borderRadius: "50px", cursor: "pointer", fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700 }}>Добавить →</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div style={{ flex: 1, padding: "32px", overflowY: "auto" }}>
                ...
                <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "20px", padding: "24px" }}>
                    {loading ? (
                        ...
                        <div style={{ textAlign: "center", padding: "60px", color: "#444" }}>
                            <div style={{ fontSize: "48px", marginBottom: "16px" }}>👨‍✈️</div>
                            <div style={{ fontSize: "16px" }}>{search ? "Водитель не найден" : "Добавьте первого водителя!"}</div>
                        </div>
                    ) : (
                        <>
                            <div style={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 1fr 1fr 1fr 80px", gap: "0", borderBottom: "1px solid rgba(255,255,255,0.04)", paddingBottom: "12px", marginBottom: "8px" }}>
                                {["Водитель", "Машина", "Заказов", "Рейтинг", "Статус", ""].map((h, i) => (
                                    <span key={i} style={{ fontSize: "12px", color: "#444", fontWeight: 600 }}>{h}</span>
                                ))}
                            </div>
                            {filtered.map((d) => (
                                <div key={d.id} style={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 1fr 1fr 1fr 80px", gap: "0", padding: "16px 0", borderBottom: "1px solid rgba(255,255,255,0.03)", alignItems: "center", transition: "background 0.2s", borderRadius: "8px" }}
                                    ...
                                    <button onClick={() => setShowDeleteModal(d.id)} style={{ background: "rgba(255,80,80,0.1)", color: "#ff5050", border: "1px solid rgba(255,80,80,0.2)", borderRadius: "8px", padding: "6px 12px", cursor: "pointer", fontSize: "12px", fontFamily: "'Space Grotesk', sans-serif" }}>
                                        Удалить
                                    </button>
                                </div>
                            ))}
